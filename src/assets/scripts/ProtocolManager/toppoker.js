...
const Toppoker = function() {
  ...
  toppoker.socketState = function() {
    if(!socket) return 3;
    return socket.readyState;
  }

  toppoker.isConnecting = function() {
  ...
  var _clearAllTimers = function() {
    ...
    for(var _id in callbackTimers) {
      clearTimeout(callbackTimers[_id]);
    }
    callbackTimers = {};
  };
  ...
  toppoker.request = function(route, msg, cb) {
    ...
    if(cb) {
      var _reqId = reqId;
      callbackTimers[_reqId] = setTimeout(function() {
        var _cb = callbacks[_reqId];
        delete callbacks[_reqId];
        delete callbackTimers[_reqId];
        delete routeMap[_reqId];
        if(typeof _cb === 'function') {
          console.warn('[Toppoker] Request timeout >>', route, '>> reqId:', _reqId);
          _cb({ code: -1, error: 'Request timeout' });
        }
      }, REQUEST_TIMEOUT_MS);
    }
  };
  ...
  var processMessage = function(toppoker, msg) {
    if(!msg.id) {
      // server push message
    ...
    if(callbackTimers[msg.id]) {
      clearTimeout(callbackTimers[msg.id]);
      delete callbackTimers[msg.id];
    }
  ...
  var processMessageBatch = function(toppoker, msgs) {
    for(var i=0, l=msgs.length; i<l; i++) {
      processMessage(toppoker, msgs[i]);
    }
  };
